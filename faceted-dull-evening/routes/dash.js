...
var  queryData =  url.parse(req.url, true).query;
connection.query("select * from wait WHERE status='0'",function(error,result,fields){
if(!error){
...
var queryData = url.parse(req.url ,true ).query ;
connection.query("select * from city " , function (error , result , fields){
if(!error){
res.json(result);
}else{
res.json(error);
...
var queryData = url.parse(req.url ,true).query ;
connection.query("INSERT INTO city (namecity) VALUES('"+queryData['namecity']+"')",function (error ,result,fields){
if(!error){
res.json(result);
}else{
res.json(error);
...
router.post('/insert_tag', function(req, res, next) {
var queryData=url.parse(req.url,true).query; 
 connection.query("INSERT INTO serach ("+queryData['sql_key']+") VALUES ("+queryData['sql_value']+")  ",function(error,results,fields){
    if(!error){
   connection.query("select id from serach ORDER BY id DESC  " , function (error , result , fields){
	 res.json(result);
})
//       res.json(results);
    }else{
